...
- `GET /api/v1/campanhas/{id}/publishers` — `{items, total}` dos publishers (substituiu o removido `/{id}/media-sources`). Cada media source vem como `{id, name, active, deactivated_reason, deactivated_at}`
- `PATCH /api/v1/campanhas/publishers/media-sources/{ms_id}` — ativa/desativa uma media source (PID). Body `{active, reason?}`: desativar EXIGE `reason` (400 se vazio), reativar limpa `reason`+`deactivated_at`. Usado pelo toggle na tela de detalhe (modal de justificativa obrigatoria)
