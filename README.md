...
- `PATCH /api/v1/campanhas/{id}` — atualiza; arrays (`eventos_pagos`/`apps`/`publishers`) fazem replace total quando enviados. Se o payload remove PIDs existentes, responde `409 {detail:{message, pids_removidos:[{publisher_nome, media_source}]}}` a menos que venha `confirmar_remocao_pids: true` (form mostra modal de confirmacao)
