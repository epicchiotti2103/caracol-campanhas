...
- `GET /api/v1/campanhas/{id}/pause-windows` — janelas de pausa do mes de referencia (pause-log) `{ mes_referencia?, windows: [{inicio, fim|null, reason?}], dias_ativos?, dias_no_mes? }`. Exibido na secao "Historico de pausa" do detalhe (`PauseWindowsView`, via helper `lib/pause-windows.ts`). ⚠ **Contrato pendente de confirmacao com o tracker** (slug `campanhas-pause-log`)
