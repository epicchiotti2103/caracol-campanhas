...
- [x] **Fase 4.21 — agregado de pagamento por publisher no mes** (14/07): nova tela `/pagamentos` (link "Pagamentos" na navbar, `Wallet`) que responde "quanto pagar a cada publisher no MES somando TODAS as campanhas fechadas" — base pra emissao da NF do publisher (que precisa do total do mes, nao campanha a campanha). Seletor de mes (reusa `/campanhas?months_available=1`) → lista de publishers ordenada pelo backend, cada linha com **total a pagar por moeda** (`totalsLabel`, USD primeiro; NUNCA soma entre moedas) + count de campanhas, com **drill-down** expansivel mostrando as parcelas (campanha codigo/nome + badge travado/fechado + valor na moeda da parcela + link "Abrir"). Dois big numbers: total geral por moeda + nº de publishers. Aviso ambar quando ha parcela de fechamento nao travado. O valor e o `spend_final` JA persistido pelo fechamento (cap/exclusao por pausa/excedente aprovado ja embutidos) — **front nao recalcula nada** e a **partilha Wave (1/3) NAO se aplica** (publisher recebe cheio). Tipos novos em `types/index.ts`: `PublisherPagamentoItem`, `PublisherPagamento`, `PublishersPagamentoResponse`. Tolerante: 404/rede → estado "indisponivel" (sem erro vermelho) enquanto a rota nao existe. **Backend (slug `fechamento-publishers-agregado`) e job do subagente `tracker`** — front implementado contra o contrato abaixo; confirmar shape quando o tracker reportar.
  - **Contrato pedido ao tracker**: `GET /api/v1/campanhas/fechamento/summary/publishers?month=YYYY-MM` (path sob `/summary/` pra NAO colidir com `/fechamento/{fechamento_id}`). Agrega `campanhas_fechamento_publishers.spend_final` (valor ja calculado — sem recompute) de TODOS os fechamentos PERSISTIDOS do mes (locked ou nao), agrupando por publisher. Resolve `supplier_id` casando `publisher_name` → cadastro (`campanhas_publishers`, case-insensitive, mesmo match do `poAcordadoByPublisher` do modal); sem match → `supplier_id: null` + nome do fechamento. Agrupa por (publisher, moeda) — `totals_by_moeda`. **Aplica a MESMA visibilidade do `/fechamento/summary`** (hub admin vê tudo; user comum só owner+membro). **NUNCA aplica partilha Wave** (payout do publisher é o `spend_final` cheio). Response: `{month, publishers:[{supplier_id, publisher_name, totals_by_moeda:{USD?,BRL?}, campanhas_count, items:[{campanha_id, campanha_codigo, campanha_nome, fechamento_id, locked, moeda, spend_final}]}], grand_total_by_moeda:{USD?,BRL?}}`.
- [x] **Fase 4.22 — lista plana global (media sources / export Excel)** (17/07): nova tela `/media-sources` (link "Media Sources" na navbar, `Table2`) que reproduz o fluxo antigo do Google Sheets que o Elio sentia falta — todas as campanhas do mes numa **tabela plana unica**, **uma linha por combinacao** (campanha × publisher × media source × evento/payout), pra filtrar por campanha e exportar pro Excel (PROCV). Colunas: Codigo, Campanha, Status, Publisher, Media source, Ativa, Evento, Payout, Moeda, Cap. **Payout vai como NUMERO CRU + Moeda em coluna propria** (nao "R$ 12,50") pra nao matar o PROCV/soma. Filtro **multi-select de campanha** (dropdown `<details>`, vazio = todas; reseta ao trocar mes) + busca por texto (publisher/media source/campanha/evento). Escopo **POR MES** (seletor igual `/campanhas`; cada mes = snapshot → sem escopo double-conta). **Export pega o conjunto FILTRADO COMPLETO** (nao ha paginacao/virtualizacao — poucas centenas de linhas/mes): botao **CSV** (dialeto pt-BR: delimitador `;`, decimal virgula, **BOM UTF-8**, escape de `;`/aspas/quebra; `Blob` + download) e **Copiar** (TSV pro clipboard via `navigator.clipboard`, decimal virgula, tabs; cola direto no Excel). Tolerante a 404/rede → estado "indisponivel" sem erro vermelho (mesmo padrao do `/pagamentos`). Tipos novos em `types/index.ts`: `CampanhaFlatRow`, `CampanhaFlatListResponse`. **Backend (slug `campanhas-flat-list`) e job do subagente `tracker`** — front implementado contra o contrato abaixo; confirmar shape quando o tracker reportar.
  - **Contrato pedido ao tracker**: `GET /api/v1/campanhas/summary/flat-list?month=YYYY-MM` (path sob `/summary/` pra NAO colidir com `GET /campanhas/{id}` — declarar a rota estatica ANTES da param). Denormaliza as campanhas do mes: **LEFT JOIN** — publisher SEM media source (ou SEM payout) TEM que aparecer com a celula vazia (no minimo 1 linha por publisher); cross-product (media_source × evento/payout) por publisher. **Escopo POR MES obrigatorio** (sem `month` double-conta a mesma campanha entre meses). Aplica a **MESMA visibilidade** do resto (hub admin ve tudo; user comum so owner+membro). **NUNCA aplica partilha Wave** — `payout` e o do cadastro (`campanhas_publisher_payouts`), cheio. Resolve `moeda` = moeda do publisher; `cap_*` = cap GERAL vigente do publisher (informativo). Response: `{month, rows:[{campanha_id, campanha_codigo, campanha_nome, campanha_status, mes_referencia, publisher_id, publisher_nome, supplier_id, moeda, media_source (nome/PID|null), media_source_ativa (bool|null), evento_nome (|null), payout (num|null), cap_tipo ("mensal"|"diario"|null), cap_unidade ("eventos"|"usd"|null), cap_valor (num|null)}]}`.
- [x] **Fase 4.23 — pagamento do publisher: vazio sem PO + memorial do calculo** (04/08): o backend (task `pagamento-po-x-eventos` do `tracker`, em paralelo) parou de pre-preencher o Pagamento com o `spend_real` da campanha (que estava em BRL carimbado como USD — erro de ~10x) e passou a sugerir `Σ(qty_evento × PO)` **na moeda do publisher**, mandando **`spend_final: null`** quando nao da pra calcular (publisher sem PO cadastrado). No `components/campanha-fechamento-modal.tsx`: (1) **`spend_final` null renderiza o input VAZIO** com placeholder `sem PO` (zero e valor legitimo de pagamento e NAO pode virar `0,00` fake) — `PublisherRow.pagamento_sem_po` marca so as rows que vieram nulas do backend (row adicionada a mao mantem placeholder `0,00`); (3) campos novos OPCIONAIS do backend: `pagamento_base: [{evento_nome, qty, payout, subtotal}]` vira tooltip com o memorial da conta (`pagamentoBaseTitle`, no wrapper da celula pra funcionar tb com fechamento travado) e `po_parcial: true` vira badge ambar "PO parcial" (publisher com >1 evento e so parte com PO → valor incompleto). **Ausencia dos campos nao muda nada** — a tela funciona identica antes e depois do backend subir. (4) `handleSave` deixou de bloquear com "Spend final invalido" quando o campo esta VAZIO: vazio vai como `0` no payload (mesma leitura que `publishersSumByMoeda`/`custoPublisherLive` ja faziam); valor preenchido invalido/negativo continua bloqueando. Tipos: `FechamentoPublisher.spend_final` alargado pra `number | null`, +`pagamento_base?`/`po_parcial?`, novo `PagamentoBaseItem`. **Historico — equivalente em R$ REMOVIDO a pedido do user (04/08)**: houve um corte intermediario que mostrava, ao lado do input de Pagamento, o equivalente na moeda da campanha (`~R$ 1.237,50`) convertido pelo `fx_rate`, e que pra viabilizar isso tirou o campo **Cambio** de dentro do bloco Wave, deu default 5,5 e passou a gravar `fx_rate` em TODO fechamento (commits `6f5d238` + `2c5b0e7`, Fase 4.24). O user testou e vetou ("nao gostei desse real do lado do po, ele so atrapalhou") → **revertido no commit `%s`**. Sem o display, gravar `fx_rate` fora do Wave nao tinha contrapartida e ainda arrastava efeito colateral: o `_custo_publisher_convertido` do backend passaria a converter onde antes somava cru, mexendo em Lucro Bruto/Liquido no `/desempenho` e no Gerencial. **Estado atual = o de sempre**: campo Cambio SO dentro do bloco de partilha Wave, sem default, e `handleSave` com `fx_rate: isRevenueShare ? fxRateNumber : null`. O que ficou desta fase e so o (1), (3) e (4) acima.
- [x] **Fase 4.24 — resumo de PIDs pausados no fechamento** (10/09): o `components/campanha-fechamento-modal.tsx` ganhou, no topo da secao Publishers, o bloco colapsavel **`PidsPausadosResumo`** — consolidado de TODOS os PIDs (media sources) inativos da campanha, lido do `publishers_cadastrados[].media_sources` (nao ha rota nova; os campos `active`/`deactivated_reason`/`deactivated_at`/`deactivated_registered_at` ja vinham no GET do fechamento). Existe porque o Elio confere o fechamento na mao e o `InactiveMediaSources` inline (11px, por publisher) so aparece pra publisher que TEM linha no fechamento — publisher pausado o mes inteiro nao tem linha e sumia da tela. Grupos, pelo mes de referencia (`month` do modal, comparacao por `YYYY-MM` LOCAL via helper `ymLocal`, que trata data pura `YYYY-MM-DD` sem passar pelo `Date` pra nao deslocar de fuso): **`Pausado durante <mes>`** (badge ambar — numeros parciais), **`Ja estava pausado`** (muted — nao rodou no mes), **`Pausado depois do mes`** (muted — so aparece ao abrir fechamento antigo de PID pausado depois; grupo alem do pedido, porque nao cabia nos outros dois sem mentir) e **`Sem data registrada`** (cinza — dado faltando, tem que ficar visivel). Cada linha: publisher · PID (mono) · data dd/mm/aaaa · motivo truncado com `title` completo; `deactivated_registered_at` vai no `title` da linha. Ordenacao por data desc (sem data, por publisher/PID). Cabecalho sempre mostra os contadores por grupo (a info sobrevive ao colapso); default aberto ate 10 itens. Zero PID inativo → nao renderiza nada. O `InactiveMediaSources` inline continua como estava, so passou a mostrar badge **`sem data`** em vez de omitir o trecho quando `deactivated_at` e nulo. **Puramente INFORMATIVO — nenhuma regra de calculo mudou**: quem exclui dias do fechamento e a pausa da CAMPANHA (`campanhas_status_log`/`status_windows`), nao a pausa de PID. O `deactivated_at` vinha nulo por bug de backend (o clone mensal da campanha zerava a data) — correcao + backfill sao job do subagente `tracker`.
