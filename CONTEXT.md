...
- [x] **Fase 4.23 — pagamento do publisher: equivalente em R$ + vazio sem PO** (04/08): o backend (task `pagamento-po-x-eventos` do `tracker`, em paralelo) parou de pre-preencher o Pagamento com o `spend_real` da campanha (que estava em BRL carimbado como USD — erro de ~10x) e passou a sugerir `Σ(qty_evento × PO)` **na moeda do publisher**, mandando **`spend_final: null`** quando nao da pra calcular (publisher sem PO cadastrado). No `components/campanha-fechamento-modal.tsx`: (1) **`spend_final` null renderiza o input VAZIO** com placeholder `sem PO` (zero e valor legitimo de pagamento e NAO pode virar `0,00` fake) — `PublisherRow.pagamento_sem_po` marca so as rows que vieram nulas do backend (row adicionada a mao mantem placeholder `0,00`); (2) **equivalente na moeda da campanha** ao lado do input, texto pequeno read-only (`~R$ 1.226,25`), recalculado a cada tecla via `pagamentoEquivalente(input, moeda)` — **so** quando publisher e USD, campanha e BRL e `fx_rate > 0` (o campo e definido como "US$ → moeda do fechamento"; fora dessa direcao seria chute de cambio → nao mostra nada). Nunca entra no payload; (3) campos novos OPCIONAIS do backend: `pagamento_base: [{evento_nome, qty, payout, subtotal}]` vira tooltip com o memorial da conta (`pagamentoBaseTitle`, no wrapper da celula pra funcionar tb com fechamento travado) e `po_parcial: true` vira badge ambar "PO parcial" (publisher com >1 evento e so parte com PO → valor incompleto). **Ausencia dos campos nao muda nada** — a tela funciona identica antes e depois do backend subir. (4) `handleSave` deixou de bloquear com "Spend final invalido" quando o campo esta VAZIO: vazio vai como `0` no payload (mesma leitura que `publishersSumByMoeda`/`custoPublisherLive` ja faziam); valor preenchido invalido/negativo continua bloqueando. Tipos: `FechamentoPublisher.spend_final` alargado pra `number | null`, +`pagamento_base?`/`po_parcial?`, novo `PagamentoBaseItem`. **Limitacao (RESOLVIDA na Fase 4.24)**: no primeiro corte o equivalente em R$ so aparecia em fechamento de **partilha Wave**, porque o `handleSave` gravava `fx_rate: isRevenueShare ? fxRateNumber : null` — fora do Wave o cambio era persistido como null.
- [x] **Fase 4.24 — cambio em TODO fechamento (default 5,5)** (04/08): o campo **Cambio (US$ → moeda da campanha)** saiu de dentro do bloco de partilha Wave e passou a ser renderizado em **qualquer fechamento**, na coluna direita do grid financeiro (abaixo de "Moeda"). **Mesmo campo, mesmo estado `fxRate`** — o bloco Wave nao tem mais input proprio, so uma nota read-only mostrando o valor em uso e apontando pro campo acima (nada duplicado). **Default `5,5`** (`FX_RATE_DEFAULT` = 5.5 → `FX_RATE_DEFAULT_INPUT` = "5,5000") quando o fechamento nao tem `fx_rate` salvo — stub novo OU fechamento antigo gravado com null; com valor salvo, o salvo ganha (helper `initialFxRate`, aplicado no load E no re-hidratar pos lock/unlock). **O default NAO se aplica a fechamento Wave** (`is_revenue_share` ou `imposto_pct != null` — espelha o `_revenue_share_active` do backend): la o cambio alimenta a conta custo/lucro/margem ja calculada e persistida, entao injetar 5,5 faria a previa (inclusive de fechamento TRAVADO, que so exibe numero congelado) divergir do que esta gravado. Wave sem cambio segue vazio, exatamente como antes. O `handleSave` passou a mandar **`fx_rate: fxRateNumber` sempre** (era `isRevenueShare ? fxRateNumber : null`). Com isso o `(~R$ ...)` ao lado do Pagamento (Fase 4.23) funciona em fechamento normal — ex. CMP-016 junho/26 (campanha BRL, publishers USD): `$225,00 x 5,5 = ~R$ 1.237,50`. **`imposto_pct` continua gated por `isRevenueShare` e NAO pode deixar de ser** — o backend deriva `is_revenue_share` de `imposto_pct is not None` (`_revenue_share_active`), entao mandar imposto em fechamento normal converteria ele em Wave. **Conta do Wave intacta**: `custoPublisherLive`/`waveCalc`/`margemCaracol` nao foram tocados e o campo alimenta o mesmo estado de antes. **Efeito colateral cross-app conhecido**: o backend calcula `custo_publisher_total`/`lucro_*` pra TODO fechamento via `_custo_publisher_convertido`, que sem `fx_rate` somava USD e BRL crus (fallback). Fechamento nao-Wave **re-salvo** a partir de agora passa a ter o custo convertido de verdade (USD x 5,5) — corrige a soma de moedas misturadas, mas muda os numeros de Lucro Bruto/Liquido no `/desempenho` e no **Gerencial** (`gerencial.py::_fechamento_custo_publisher`, mesma logica). Nenhum fechamento ja gravado muda sozinho — so no proximo save.
