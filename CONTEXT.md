...
- [x] **Fase 4.13 — historico de pausa/reativacao (pause-log)** (25/06): a **reativacao** da campanha agora pede DATA EFETIVA (antes nao pedia nada). O botao "Reativar campanha" abre o `DateOnlyModal` (so data, default hoje — espelha o `ReasonDateModal` sem o campo de motivo) e o `POST /campanhas/{id}/unpause` passou a enviar body `{ effective_at: "YYYY-MM-DD" }`. O detalhe (`app/campanhas/[id]/page.tsx`) ganhou a secao **"Historico de pausa (mes de referencia)"** que lista as janelas do mes ("Pausada de DD/MM ate DD/MM", ou "ate agora" se ainda pausada, + motivo) e, quando o backend retorna, "X de Y dias ativos no mes" (`PauseWindowsView`). As janelas vem do helper isolado `lib/pause-windows.ts` (`fetchPauseWindows`, `GET /campanhas/{id}/pause-windows`), tolerante a backend ausente (404/rede -> null, secao some). Tipos novos em `types/index.ts`: `CampanhaPauseWindow`/`CampanhaPauseWindowsResponse`. Componente novo `components/date-only-modal.tsx`. **Backend (slug `campanhas-pause-log`) e job do subagente `tracker`** — log de pausa/reativacao, `unpause` aceitando `effective_at`, e o endpoint de janelas + dias ativos. ⚠ **Contrato do endpoint de janelas PENDENTE DE CONFIRMACAO** — front implementado contra o shape provavel (`{mes_referencia?, windows:[{inicio, fim|null, reason?}], dias_ativos?, dias_no_mes?}`); ajustar `lib/pause-windows.ts` + o tipo quando o tracker reportar.
